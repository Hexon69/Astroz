...
class Extra(commands.Cog):
    def __init__(self, bot):
        ...
        self.db = self.connection["Universal"]["servers"]    

    @commands.hybrid_command(name="statistics",aliases=["st"],usage="stats")
    @blacklist_check()
    async def stats(self, ctx):
        """Shows some usefull information about Universal™"""
    ...
    async def invite(self, ctx: commands.Context):
        embed = discord.Embed(title="**Invite Link:**", description = "> • [Click Here To Invite Astroz To Your Server](https://discord.com/oauth2/authorize?client_id=1012627088232165376&permissions=2113268958&scope=bot)\n> • [Click Here To Join My Support Server](https://discord.gg/HP3gXrakcu)\n> • [Click Here To Vote Astroz](https://discord.com/oauth2/authorize?client_id=1012627088232165376&permissions=2113268958&scope=bot)", color=0x00FFE4, timestamp=ctx.message.created_at)
        ...
        b = Button(label='Invite Me', style=discord.ButtonStyle.link, url='https://discord.com/oauth2/authorize?client_id=1012627088232165376&permissions=2113268958&scope=bot')
        hacker = Button(label='Support Server', style=discord.ButtonStyle.link, url='https://discord.gg/QzXxrh9Q')
        #mohit = Button(label='Vote Me', style=discord.ButtonStyle.link, url='https://discord.com/oauth2/authorize?client_id=1021416292185546854&permissions=2113268958&scope=bot')
        view = View()
        view.add_item(b)
        view.add_item(hacker)
...
**Threads:** {len(guild.threads)}
    ...
    async def userinfo(self, ctx: commands.Context, member: discord.Member = None):
      ...
      #embed.add_field(name="__**URLs:**__", value=f"**Avatar URL:** [Click Me]({member.avatar})\n**Guild Avatar URL:** [Click Me]({member.display_avatar})")        
    ...
    async def list_inrole(self, ctx, role: discord.Role):
      guild = ctx.guild
      embed = discord.Embed(title=f"List of Members in {role} - {len(role.members)}", color=0x00FFE4, description="")
    ...
    async def ping(self, ctx):
        ...
        embed = discord.Embed(title='Ping',
                          color=0x00FFE4,
                          description="**%s Shard **`%s`** latency: `%sms`\n%s Database latency: `%sms`**"
            % (client_emoji, ctx.guild.shard_id, ping_, db_emoji, db))
        embed.set_thumbnail(url ="https://media.discordapp.net/attachments/1036538198236614676/1037664035186954270/blue_circle.jpg")
        embed.set_author(name="Astroz", icon_url=f"https://media.discordapp.net/attachments/1036538198236614676/1037664035186954270/blue_circle.jpg")
        embed.set_footer(text=f"Requested By {ctx.author}", icon_url=f"{ctx.author.avatar}")
        await ctx.send(embed=embed)



    @commands.hybrid_command(name="firstmsg",
        help=
    ...
    @commands.hybrid_command(name="note",help="Creates a note for you",usage="Note <message>")
    ...
    async def notes(self, ctx):
        ...
        if stats is None:
            ...
            embed = discord.Embed(
                title="Notes", description=f"Here are your notes", color=0x00FFE4
            )
            x = notedb.find({"id": ctx.author.id})
            z = 1
            async for i in x:
    ...
    @commands.hybrid_command(name="trashnotes",help="Delete the notes , it's a good practice",usage="Trashnotes")
    @blacklist_check()
    ...
    async def _badges(self, ctx, user: Optional[discord.User] = None):
      ...
      if badges == []:
        embed2 = discord.Embed(description="**__BADGES__** <a:lgn_nitrobadgesroll:1002369377427013682>\n\n`No Badge Available`", color=0x00FFE4)
        embed2.set_author(name=f"Profile For {mem}", icon_url = "https://media.discordapp.net/attachments/1036538198236614676/1037664035186954270/blue_circle.jpg")
        ...
        embed = discord.Embed(description="**__BADGES__** <a:lgn_nitrobadgesroll:1002369377427013682>\n\n",color=0x00FFE4)
        embed.set_author(name=f"Profile For {mem}",icon_url="https://media.discordapp.net/attachments/1036538198236614676/1037664035186954270/blue_circle.jpg")

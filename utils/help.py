...
class View(discord.ui.View):
	...
	def hacker(self):
		inv = Buttons(label='Invite Me', style=discord.ButtonStyle.link, url='https://discord.com/oauth2/authorize?client_id=1012627088232165376&permissions=2113268958&scope=bot')
		sup = Buttons(label='Support Server', style=discord.ButtonStyle.link, url='https://discord.gg/kEzNhqNKyg')
		hack = [inv, sup]
		for button in hack: self.add_item(button)
		return hack
	...
	def add_buttons(self):
		self.startB = Buttons(emoji="⏪", style=discord.ButtonStyle.grey, command=self.set_page, args=0, ctx = self.ctx)
		self.backB = Buttons(emoji="⏮", style=discord.ButtonStyle.grey, command=self.to_page, args=-1, ctx = self.ctx)
		self.nextB = Buttons(emoji="▶️", style=discord.ButtonStyle.grey, command=self.to_page, args=+1, ctx = self.ctx)
		self.endB = Buttons(emoji="⏩", style=discord.ButtonStyle.grey, command=self.set_page, args=len(self.options)-1, ctx = self.ctx)
		self.quitB = Buttons(emoji="⏹", style=discord.ButtonStyle.grey, command=self.quit, ctx = self.ctx)
	...
	def gen_embeds(self):
		options, embeds = [], []
		options.append(discord.SelectOption(label="Home", emoji='<a:dev:979309038888189982>'))
